...
def anomaly_app():
	...
	test_df = pd.read_csv("data/test_score_df.csv",index_col = 0)
	anomalies = pd.read_csv("data/anomaly_dataset.csv",index_col = 0)
	test = pd.read_csv("data/test_dataset_anomaly.csv",index_col = 0)
	...
	if choice == "Units":
		with col1:
			...
			with st.beta_expander("Test_error"):
				img1 = Image.open("Images/mae_test.png")
				st.image(img1)
		with col2:
			with st.beta_expander("Anomalies"):
				st.dataframe(anomalies)
			with st.beta_expander("Train Loss"):
				img2 = Image.open("Images/mae_train.png")
				st.image(img2)			
		with st.beta_expander("Anomaly Threshold"):
			img3 = Image.open("Images/threshold_graph.png")
			st.image(img3)	
	if choice == "App1":
		# with col1:
		# 	with st.beta_expander("Anomaly on Test Data"):
